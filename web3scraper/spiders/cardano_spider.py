...
class CardanoSustainabilitySpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        # Extract TextField (input fields)
        ...
        # Ensure that text_fields always returns a list (even if empty)
        if not text_field_values:
            text_field_values = []  # Default empty list if no text fields found

        # Add company name and URL to the item
        yield {
            'company_name': 'Cardano',  # Include company name explicitly
            'network': 'cardano',
            'sustainability_info': {
                'headings': headings,
                ...
                'links': links,
                'text_fields': text_field_values  # Add text field values here, even if empty
            },
            'url': response.url
        }
